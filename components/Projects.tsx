...
export const Projects: React.FC<ProjectsProps> = ({ db, setDb, currentUser, theme }) => {
  ...
  const calculateWorkingDays = (startStr: string, endStr: string) => {
    ...
    if (start > end) return `0 ${t('dashboard.days')}`;
    let count = 0;
    ...
    return `${count} ${count === 1 ? t('dashboard.day') : t('dashboard.days')}`;
  };
  ...
  const removePhoto = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    if (confirm(t('projects.confirmDeletePhoto'))) {
      setPhotoUrl('');
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (db.projects.some((p: Project) => p.code === finalCode && p.id !== editingProject?.id)) {
      alert(t('projects.duplicateCode'));
      return;
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <h1 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight transition-colors">{t('projects.title')}</h1>
      ...
      <div className="bg-white dark:bg-[#1e293b] p-6 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 flex flex-wrap gap-4 items-center transition-colors">
        ...
        >
          ...
          {Object.values(ProjectStatus).map(s => <option key={s} value={s}>{t(`status.${s.toLowerCase()}`)}</option>)}
        </select>
        <select
          className="bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-indigo-500 font-bold cursor-pointer transition-colors"
          value={clientFilter}
        ...
        >
          <option value="ALL">{t('projects.clientFilter')}</option>
          {[...db.clients]
      ...
      <div className="bg-white dark:bg-[#1e293b] rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 overflow-hidden transition-colors">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead className="sticky top-0 z-10">
              <tr className="bg-slate-50 dark:bg-[#2a374a] border-b border-slate-200 dark:border-slate-800 transition-colors">
                ...
                <th className="px-2.5 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center w-12">{t('projects.miniature')}</th>
                <th className="px-2.5 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest min-w-[450px]">{t('projects.name')}</th>
                <th className="px-2.5 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest w-32 text-center">{t('projects.code')}</th>
                <th className="px-2.5 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest w-40">{t('projects.client')}</th>
                <th className="px-2.5 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center w-36">{t('projects.status')}</th>
                <th className="px-2.5 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest w-32">{t('projects.responsible')}</th>
                <th className="px-2.5 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center w-32">{t('projects.deadline')}</th>
                ...
                return (
                  <tr key={project.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/20 transition-colors group relative border-l-4 border-transparent">
                    ...
                    <td className="px-2.5 py-4">
                      ...
                      >
                        {client?.name || t('common.notInformed')}
                      </button>
                    </td>
                    <td className="px-2.5 py-4 text-center">
                    ...
                    <td className="px-2.5 py-4">
                      {assignee ? (
                        ...
                        <span className="text-[11px] text-slate-600 italic">{t('common.notInformed')}</span>
                      )}
                    </td>
                    <td className="px-2.5 py-4 text-center">
                      <div className="flex flex-col items-center space-y-0.5">
                        ...
                        <div className="text-[8px] font-black text-slate-500 dark:text-slate-600 bg-slate-100 dark:bg-slate-800/60 px-2 py-0.5 rounded-full border border-slate-200 dark:border-slate-700/50 uppercase tracking-tighter mt-0.5 transition-colors">
                          {workingDays.split(' ')[0]} {t('projects.workingDays')}
                        </div>
                      </div>
                    </td>
          ...
          {filteredProjects.length === 0 && <div className="py-24 text-center text-slate-600 font-black uppercase tracking-[0.2em] text-xs">{t('projects.noProjects')}</div>}
        </div>
      </div>
          ...
          <div className="bg-white dark:bg-[#0f172a] rounded-[40px] shadow-2xl w-full max-w-2xl h-[90vh] flex flex-col overflow-hidden border border-slate-200 dark:border-white/5 transition-all duration-500">
            <div className="px-8 py-6 bg-slate-50 dark:bg-slate-900/50 border-b border-slate-100 dark:border-white/5 flex items-center justify-between transition-colors">
              <h3 className="text-lg font-black text-slate-900 dark:text-white uppercase tracking-tight transition-colors">
                {currentUser.role === UserRole.VIEWER ? t('projects.view') : (editingProject ? t('projects.edit') : t('projects.new'))}
              </h3>
            ...
            <form onSubmit={handleSave} className="flex-1 overflow-y-auto custom-scrollbar p-8 space-y-8 dark:bg-[#0f172a] transition-colors">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-2">
                <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-2xl border border-slate-100 dark:border-slate-700/50 flex items-center space-x-3 transition-colors">
                  ...
                  <div>
                    <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest leading-none mb-1">{t('projects.projectCode')}</p>
                ...
                <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-2xl border border-slate-100 dark:border-slate-700/50 transition-colors">
                  <label className="block text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1.5 ml-1 transition-colors">{t('projects.id')}</label>
                  <input
                    type="text"
                    value={customCode}
              ...
              <div className="bg-slate-50 dark:bg-slate-900/40 p-5 rounded-[24px] border border-slate-100 dark:border-white/5 space-y-4">
                <label className={`flex items-center space-x-3 cursor-pointer group ${currentUser.role === UserRole.VIEWER ? 'pointer-events-none opacity-60' : ''}`}>
                  ...
                  <input
                    ...
                    className="hidden"
                  />
                  <span className="text-[10px] font-black text-slate-600 dark:text-slate-400 uppercase tracking-widest group-hover:text-indigo-500 transition-colors">{t('projects.addPrefix')}</span>
                </label>
                ...
                {usePrefix && (
                  <div className="animate-in slide-in-from-top-2 duration-300">
                    <input
                      ...
                      placeholder={t('projects.prefixPlaceholder')}
                    ...
                    />
                    <p className="text-[9px] font-bold text-indigo-500/60 uppercase tracking-wider mt-2 ml-1">{t('projects.prefixHint')}</p>
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-5">
                  <div>
                    <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2 px-1">{t('projects.client')} *</label>
                  ...
                  <div>
                    <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2 px-1">{t('projects.responsible')} *</label>
                  ...
                  <div>
                    <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2 px-1">{t('projects.name')} *</label>
                    <input type="text" required value={name} disabled={currentUser.role === UserRole.VIEWER} onChange={(e) => setName(e.target.value)} className="w-full px-4 py-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-sm text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-indigo-500 outline-none font-medium transition-colors disabled:opacity-60" placeholder={t('projects.namePlaceholder')} />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2 px-1 transition-colors">{t('projects.revision')}</label>
                    ...
                    <div>
                      <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2 px-1 transition-colors">{t('projects.status')}</label>
                ...
                <div className="space-y-5">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2 px-1 transition-colors">{t('projects.startDate')}</label>
                    ...
                    <div>
                      <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2 px-1 transition-colors">{t('projects.deliveryDate')}</label>
                  ...
                  <div>
                    <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2 px-1 transition-colors">{t('projects.image')}</label>
                      ...
                      {photoUrl ? (
                        <div className="relative w-full h-full">
                          ...
                          <div className="absolute inset-0 bg-slate-900/80 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center space-y-3 p-4">
                            {currentUser.role !== UserRole.VIEWER && (
                              <>
                                <button type="button" onClick={() => fileInputRef.current?.click()} className="w-full py-2 bg-indigo-600 text-white text-[10px] font-black uppercase tracking-widest rounded-lg shadow-lg hover:bg-indigo-700 transition">{t('projects.changePhoto')}</button>
                                <button type="button" onClick={removePhoto} className="w-full py-2 bg-rose-600 text-white text-[10px] font-black uppercase tracking-widest rounded-lg shadow-lg hover:bg-rose-700 transition">{t('projects.removeImage')}</button>
                              </>
                            )}
                            {currentUser.role === UserRole.VIEWER && (
                        ...
                        <div className="flex flex-col items-center pointer-events-none text-center px-4">
                          ...
                          <span className="text-[10px] font-black text-slate-600 uppercase tracking-widest text-center">{t('projects.clickToUpload')}</span>
                        </div>
                      )}
                ...
                <div className="space-y-3 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar transition-colors">
                  {subtasks.length === 0 ? (
                    <div className="py-8 text-center bg-slate-50 dark:bg-slate-900/30 rounded-2xl border-2 border-dashed border-slate-100 dark:border-slate-800/50 transition-colors">
                      <p className="text-[10px] text-slate-400 dark:text-slate-600 font-bold uppercase tracking-widest leading-loose">{t('projects.noSubtasks')}</p>
                    </div>
                  ) : (
                    subtasks.map((st) => (
                        ...
                        <div className="flex flex-col space-y-4">
                          <div className="flex items-center space-x-3">
                            <input
                              ...
                              placeholder={t('projects.subtaskNamePlaceholder')}
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 items-end">
                            <div className="col-span-1 md:col-span-1">
                              <label className="block text-[8px] font-black text-slate-400 dark:text-slate-600 uppercase mb-1.5 ml-0.5 transition-colors">{t('projects.responsible')}</label>
                              <select
                                value={st.assigneeId || ''}
                                disabled={currentUser.role === UserRole.VIEWER}
                            ...
                            <div className="col-span-1 md:col-span-1">
                              <label className="block text-[8px] font-black text-slate-400 dark:text-slate-600 uppercase mb-1.5 ml-0.5 transition-colors">{t('projects.status')}</label>
                              <select
                                value={st.status}
                                disabled={currentUser.role === UserRole.VIEWER}
                            ...
                            <div className="col-span-1 md:col-span-1">
                              <label className="block text-[8px] font-black text-slate-400 dark:text-slate-600 uppercase mb-1.5 ml-0.5 transition-colors">{t('projects.startDate')}</label>
                              <input
                                type="date"
                                value={st.startDate}
                                min={startDate}
                            ...
                            <div className="col-span-1 md:col-span-1">
                              <label className="block text-[8px] font-black text-slate-400 dark:text-slate-600 uppercase mb-1.5 ml-0.5 transition-colors">{t('projects.deliveryDate')}</label>
                              <input
                                type="date"
                          ...
                          <input
                            ...
                            placeholder={t('projects.subtaskNotesPlaceholder')}
              ...
              <div>
                <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2 px-1 transition-colors">{t('projects.projectNotes')}</label>
                <textarea
                  value={notes}
                  disabled={currentUser.role === UserRole.VIEWER}
                  ...
                  placeholder={t('projects.projectNotesPlaceholder')}
                />
              </div>
              ...
              <div className="pt-8 border-t border-slate-100 dark:border-slate-800 flex space-x-4 transition-colors bg-white dark:bg-[#0f172a] sticky bottom-0 z-10">
                ...
                {currentUser.role !== UserRole.VIEWER && (
                  ...
                  >
                    {editingProject ? t('common.confirmChanges') : t('projects.create')}
                  </button>
                )}
              </div>
            </form>
            ...
            <div className="px-8 py-6 bg-slate-50 dark:bg-slate-900/50 border-b border-slate-100 dark:border-white/5 flex items-center justify-between transition-colors">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest leading-none mt-1 transition-colors">{t('clients.identificationFile')}</p>
                </div>
              </div>
              <button
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-6">
                  <div className="flex items-start space-x-4">
                    ...
                    <div>
                      <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1">{t('clients.code')}</p>
                  ...
                  <div className="flex items-start space-x-4">
                    ...
                    <div className="overflow-hidden">
                      <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1">{t('clients.email')}</p>
                      <p className="text-slate-900 dark:text-white font-bold text-sm truncate transition-colors">{viewingClient.email || t('common.notInformed')}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start space-x-4">
                    ...
                    <div>
                      <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1">{t('clients.phone')}</p>
                      <p className="text-slate-900 dark:text-white font-bold text-sm transition-colors">{viewingClient.phone || t('common.notInformed')}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-6">
                  <div className="flex items-start space-x-4">
                    ...
                    <div>
                      <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1">{t('clients.cityState')}</p>
                      <p className="text-slate-900 dark:text-white font-bold text-sm transition-colors">{viewingClient.city && viewingClient.state ? `${viewingClient.city} - ${viewingClient.state}` : t('common.notInformed')}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start space-x-4">
                    ...
                    <div>
                      <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1">{t('clients.fullAddress')}</p>
                      <p className="text-slate-900 dark:text-white font-bold text-sm leading-tight transition-colors">{viewingClient.address ? `${viewingClient.address}, nº ${viewingClient.number || t('common.noNumber')}` : t('common.notInformed')}</p>
                  ...
                  <div className="flex items-start space-x-4">
                    ...
                    <div>
                      <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1">{t('clients.status')}</p>
